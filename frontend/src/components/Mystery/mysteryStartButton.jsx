...
const mysteryStartButton = ({player}) => {
    const [clickStart, setStart] = useState(false);

    const handleStartClick = async () => {
        console.log(player.name + " clicked Start")
        const result = await axios.patch(`http://localhost:8080/gamenight/server/murderMystery/player/${player.serverCode}/${player.playerID}`)
        console.log(result)
        setStart(!clickStart)
    }
...
export default mysteryStartButton